...
export default function RefreshButton({ lastUpdated }: { lastUpdated: string }) {
  ...
  return (
    <div className="flex items-center justify-between mb-5">
      <p className="text-xs text-white/50">עודכן לאחרונה: {label}</p>
      <button
      ...
      >
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} className={`w-3.5 h-3.5 ${isPending ? 'animate-spin' : ''}`}>
